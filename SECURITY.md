...
- A **write** shield is written or planted through: the persistence shields
  (`.git/hooks`, `.git/config`, editor task files under `.vscode` / `.idea`, coding
  agents' project config dirs and files such as `.claude` / `.mcp.json`) are deliberately left readable so a build can consult them, but a write or a planted
