...
func TestInspectNewManifestFollowsDotDotThroughALink(t *testing.T) {
	...
	trust, err := InspectNew(w+"/lnk/../bento.yaml", uint32(os.Geteuid()))
	if err != nil {
		t.Fatalf("InspectNew: %v", err)
	}
...
// The zero Manifest is a host where the location was never read, not one where it
...
func TestLocationFlawsRefuseAnUnreadLocation(t *testing.T) {
	const me = 1000
	unknown := Manifest{file: fileFacts{path: "/w/m.yaml", mode: 0o600, uid: me}}
...
func TestInspectNewManifestJudgesTheDirectory(t *testing.T) {
	...
	trust, err := InspectNew(filepath.Join(dir, "bento.yaml"), uint32(os.Geteuid()))
	if err != nil {
		t.Fatalf("InspectNew: %v", err)
	}
	...
	linked, err := InspectNew(filepath.Join(link, "bento.yaml"), uint32(os.Geteuid()))
	if err != nil {
		t.Fatalf("InspectNew: %v", err)
	}
...
// trustOf is how a test reaches an inspection of an existing manifest: the location and
// the facts both come from an open handle.
func trustOf(t *testing.T, path string) Manifest {
	t.Helper()
	f, err := os.Open(path)
	if err != nil {
		t.Fatal(err)
