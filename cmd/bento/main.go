...
func cmdProfile(args []string) int {
	...
	header.WriteString("# generated by `bento profile " + scriptPath + "` — review and trim before use\n")
...
func warnStrippedShellVars(w io.Writer, scriptPath, interp string, env map[string]string) {
	...
	for _, name := range commonHostVars {
	...
	fmt.Fprintf(w, "[bento] script references host env var(s): $%s\n", strings.Join(referenced, " $"))
	fmt.Fprintln(w, "[bento] bento strips host env by default — these will expand to empty strings.")
	fmt.Fprintln(w, "[bento] pass `--env NAME=VALUE` or add `env: [NAME]` to the manifest to inherit them.")
...
// shellReferencesVar checks for $NAME or ${NAME} occurrences in src. It does
// not parse shell — false positives in comments and single-quoted strings
// are acceptable for a heads-up hint.
func shellReferencesVar(src []byte, name string) bool {
	return bytes.Contains(src, []byte("$"+name)) ||
		bytes.Contains(src, []byte("${"+name+"}")) ||
		bytes.Contains(src, []byte("${"+name+":"))
}

// emitSilentWriteWarning warns when the script wrote to /sandbox/* paths
// that aren't bound to the host — those writes look successful but vanish
// when the sandbox exits.
func emitSilentWriteWarning(w io.Writer, opens []bento.FSOpen) {
	var lost []string
	for _, o := range opens {
		if o.Write && o.OK && strings.HasPrefix(o.Path, "/sandbox/") {
	...
	fmt.Fprintln(w, "[bento] script wrote to sandbox-only paths (lost on exit — not bound to host):")
	for _, p := range lost {
		fmt.Fprintln(w, "[bento]   "+p)
	}
	fmt.Fprintln(w, "[bento] add the destination directory to the manifest's `write:` list to persist these.")
	fmt.Fprintln(w, "[bento] ─────────────────────────────────────────")
}
...
func emitZeroConfigHint(w io.Writer, scriptPath string, m *bento.Manifest, stderrTail string) {
	...
	if matchesExecBlock(stderrTail) {
		emit(
			"[bento] zero-config blocks subprocess execve. If the script needs to spawn processes,",
			"[bento]   write a manifest with `allow_exec: true` and run with `bento run <manifest>.yaml`.",
		)
		return
	}
