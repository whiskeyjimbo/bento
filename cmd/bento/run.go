...
func newRunCmd() *cobra.Command {
	...
	cmd.Flags().StringArrayVar(&acceptAliases, "accept-alias", nil, "acknowledge the credential aliases under the host tree at `path` (a snapshot or deduplicated backup) instead of refusing; repeatable; the degraded tier scans and refuses the same way, so this is needed there too")
...
type streamRefusalJSON struct {
	...
	// The grants the hook report could not read whole and why, so a consumer can tell a
	// short report from a clean one; see enforce.Result.UnresolvedHooks.
	UnresolvedHooks []unresolvedGrantJSON `json:"unresolved_hooks,omitempty"`
...
func writeRunResult(stderr io.Writer, asJSON bool, manifestPath string, p *policy.Policy, env map[string]string, res enforce.Result, notes *runNotesJSON, stream *eventStream, runErr error) error {
	...
	case errors.As(runErr, &refusal):
		...
		writeRefusalRemedy(stderr, refusal)
		return &exitError{code: bentoFailed}
	...
	if asJSON {
		stream.emitTerminal(struct {
			...
			RedirectedHooks []string `json:"redirected_hooks,omitempty"`
			// The grants the hook report could not read whole and why; see
