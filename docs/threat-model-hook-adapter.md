...
2. **`--allow` auto-approves every other `tool_input` field unread (VERIFIED BY SPIKE, low).**
   `hook.go:111-114` copies all keys through. A payload with
...
| `claudecodehook` stdout | hook -> Claude Code | `hook.go:124-133` | Claude Code runs `updatedInput.command` in a host bash |
...
| `claudecodehook/fail-open` (manifest unapproved/no extra_args/replaceable/missing) | ENFORCED `hook.go:91-106` | allow of a swappable or refusing run | reason | EXECUTION `TestHookDeniesWhatItCannotSandbox` |
