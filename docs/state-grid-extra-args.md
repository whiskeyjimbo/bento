...
| 8 | CLI trailing (profile script args) | P | HANDLED | profile puts trailing args into policy.Args (profile.go:188, 1450), so they pass `Validate` at internal/linux/profile.go:48; the CLI passes no proc.ExtraArgs, and an embedder's are admitted by `enforce.AdmitExtraArgs` in Profile | READING |
| 9 | Process.ExtraArgs | E | FIXED: both entries call `enforce.AdmitExtraArgs`; pinned by `TestRunAdmitsExtraArgsItself` and `TestProfileAdmitsExtraArgsItself` (internal/linux) - was **WRONG (forbidden direction)** | `Enforcer.Run` calls `enforce.AdmitExtraArgs` beside its Validate/RequireExpanded/RunID re-checks, so a direct embedder gets the same refusal; `Enforcer.Profile` calls it too before `command()` (args.go:814) appends them | EXECUTION (`go test ./internal/linux -run TestRunAdmitsExtraArgsItself`) |
...
| 12 | any source | degraded tier | HANDLED | same `command()` (degraded.go:228) behind the same `enforce.Run` checks, and the backend's own `AdmitExtraArgs` (cell 9) runs before either tier | READING |
