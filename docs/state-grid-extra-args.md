...
| 9 | Process.ExtraArgs | E | FIXED: both entries call `enforce.AdmitExtraArgs`; pinned by `TestRunAdmitsExtraArgsItself` (internal/linux) - was **WRONG (forbidden direction)** | `Enforcer.Run` calls `enforce.AdmitExtraArgs` beside its Validate/RequireExpanded/RunID re-checks, so a direct embedder gets the same refusal. `Enforcer.Profile` still reaches `command()` (args.go:814) without it | EXECUTION (`go test ./internal/linux -run TestRunAdmitsExtraArgsItself`) |
