...
2. The observer (`internal/observe`) attaches via `ptrace` and inspects `open`/`openat` syscall registers and memory parameters *before* the kernel processes the call.
