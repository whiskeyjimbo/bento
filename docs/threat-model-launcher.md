...
| `probecanary/systemd-run-unchecked` | same-uid (next run) | `PARTIAL` | the probes' own `systemd-run` is still PATH-resolved and executed before any preflight; a planted one runs, but cannot get a scoped run launched (`preflightLimits` refuses it) and cannot forge more than a reading `noteScopeLimits` answers from the kernel | `SILENT` | read | `internal/linux/limits.go:140`, `limits.go:450`; open as bv2-fk0ri |
