...
type sandbox struct {
	...
	// projectConfig maps each resolved write grant to a DenyWrite rule per project config
	// entry (denylist.ProjectConfig) found below it, git checkout or not. Found in
	// the same walk as nestedCheckouts.
	projectConfig map[string][]denylist.Rule
...
// cause: every git checkout and project config entry under a write grant brings its own
// shields and pins.
func checkBwrapArgCount(args []string) error {
	if len(args) <= bwrapMaxArgs {
		return nil
	}
	return fmt.Errorf("linux: this sandbox needs %d bubblewrap arguments and bubblewrap accepts at most %d; each git checkout and editor or agent config entry under a write grant adds its own shields, so grant the projects that need writing rather than a directory holding many", len(args), bwrapMaxArgs)
}
