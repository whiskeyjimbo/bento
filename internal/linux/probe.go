...
// limitsLayers decides the resource-limit layers. Both tiers now wrap the target in
// a systemd scope (the bwrap tier in Run, the degraded tier in runDegraded - the
// scope's cgroup applies the caps regardless of which namespaces exist), so a limit
// is enforced whenever a scope can be created; the cpu sub-layer additionally needs
// the cpu controller delegated. With no scope the whole layer is Unavailable and a
// cpu-limit policy is refused by it, so no separate LayerLimitsCPU is emitted.
