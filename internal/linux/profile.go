...
func (e *Enforcer) Profile(ctx context.Context, p *policy.Policy, proc enforce.Process, allowNetwork bool, denyPaths, acceptAliasesUnder []string) (obs profile.Observation, err error) {
	...
	sb, cleanup, err := newSandbox(p, e.selfPath, false, denyPaths, nil, true)
	if err != nil {
		return profile.Observation{}, err
	}
	defer cleanup()
	...
	if !p.Limits.IsZero() {
		...
		exe, cargs = wrapWithLimits(runner, bwrap, args, p.Limits, "")
	}
...
// line if the target issued an execve and an "EXECRAN" line if a spawn actually happened,
// an "EXIT <code>"
